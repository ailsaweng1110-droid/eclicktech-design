...
## 六、单条信息标准模板（观点导向，不做陈述）

### 飞书表里的存档版本（保留链接）

```markdown
title: 中文译名（保留人名/产品英文）
published_at: YYYY-MM-DD
summary: 80–150 字，观点导向（见下方结构）
url: 原文链接（飞书表归档用，不进公众号）
source: 发布方名称
importance: 1–5
```
...
### 描述写作核心规则：观点导向，不要陈述事实

| 维度 | ❌ 陈述风格（要避免）| ✅ 观点风格（要采用） |
|---|---|---|
| 第一句 | "X 报道/发布/宣布……" | **直接抛判断**——这件事的意义、你的态度 |
| 中段 | 平铺直叙地翻译 | 用数据/案例**支撑判断**，必要时解释术语 |
| 末句 | 收尾或不收 | **给读者"应该怎么想/做"**——启示、警示、建议 |

### 描述写作"三段式"模板

```text
【判断】这件事最重要的一句话/最反直觉的事实
+
【支撑】数据 / 案例 / 关键引述（顺手补本地化解释）
+
【启示】对国内/出海/设计/营销/创业团队的"应该怎么办"
```

### 示例对比

**❌ 陈述版（翻译腔）**：
> SEJ 报道一项随机现场实验：Google AI Overviews 触发时，有机点击下降 38%，但用户体验评分没有变化——这说明流失的是「低意图点击」，但商家正实实在在地损失流量。

**✅ 观点版**：
> **过去 20 年最大的流量入口正在塌。** 随机实验显示：Google 触发 AI Overviews（顶部 AI 直接给答案）时，下方网站的有机点击被砍 38%，而用户体验评分没变——用户得到 AI 答案就够了，不再点进网站。**给国内出海团队的启示**：如果你还在按传统 SEO 投入，请立刻把一半预算转向 AEO（让品牌被 AI 引用）。否则你优化的不是流量，是一艘正在沉的船。

### 创始人观点的特殊规则

- 标题不带公司/职位
- 描述首句必须交代 "X 是 [公司] [职位]"
- 第二段直接抛"这位创始人最值得记住的判断是什么"——即使他没明说，AI 也要替读者总结
- 末段给 1-2 句"对国内创业者/团队的启示"
...
10. **调 media-formatter skill** 输出公众号 HTML，**严格遵守以下两条**：
    - **去掉所有原文链接**（公众号外链对国内用户体验差，飞书表归档保留链接即可）
    - **保留【来源：xx】标记**（让读者知道信息出处）
    - 同时按第六节"观点导向三段式"重写描述（如果飞书原条目还是陈述风格）
...
用户挑要发的，手动改「已审」
↓
用户告诉我已审的条目（截图/贴标题/口述）—— 详见下方"AI 读不到飞书"
↓
我出 HTML
↓
用户手动把它们改「已排版」（防止下次重复）
...
飞书 webhook 是**单向写入**机制，AI 没有读取多维表的能力。

因此用户审完稿后，必须用以下任一方式把"已审"的条目传给 AI：

| 方式 | 用户操作 | 推荐度 |
|---|---|---|
| 直接口述 | "01 栏选 X / Y / Z；02 栏选 A / B …" | ⭐⭐⭐ 最快 |
| 复制标题 | 复制飞书表已审行的标题列粘贴给 AI | ⭐⭐ 精准 |
| 截图 | 在飞书表筛选 `状态=已审` 截图发 AI | ⭐ 兜底 |

AI 收到后会从最近一次推送的源数据（如 `_push_curated.py` 里的 `CURATED` 数组）匹配对应条目，加本地化补丁后调 `media-formatter` 输出 HTML。
