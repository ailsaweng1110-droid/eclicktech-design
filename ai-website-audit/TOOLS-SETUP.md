...
|----------------|---------|---------|------------|
| `fetch_page(url)` | Jina Reader `r.jina.ai` | 无限制（有速率限制） | 不需要 |
| `search_web(query)` | Tavily Search API | 1,000次/月 | 需要（免费，无需信用卡） |
...
部署到其他平台后，用以下清单验证运行质量：

### 输入阶段
- [ ] 模型能正确识别 `/ai-website-audit URL` 触发指令
- [ ] 工具调用名称为 `fetch_page` 和 `search_web`（非 `web_fetch`/`web_search`）
- [ ] 第一次 fetch_page 能成功返回 Markdown 内容（测试：访问 https://example.com）
- [ ] search_web 能返回 Tavily 的结构化结果

### 诊断流程
- [ ] Step 0：成功推断行业类型，输出权重（无需网络）
- [ ] Step 1：fetch_page 调用 `/llms.txt`（404 时能正常继续）
- [ ] Step 2：fetch_page 调用 3 次（首页 + 关于页 + 案例页）
- [ ] Step 3：AI 视角内容预览以 Markdown 代码块展示（标注盲区）
- [ ] Step 4+5：search_web 调用 2 次（认知测试 + 行业推荐测试）
- [ ] Step 6～8：本地计算，无网络请求

### 输出阶段
- [ ] 核心结论卡以 Markdown 表格输出（含综合分、三维度得分、优化空间）
- [ ] 评分明细以 Markdown 表格输出（设计6项+内容8项+技术26项）
- [ ] 优化建议以分层列表输出（P1 / P2 / P3）
- [ ] 完整 HTML 报告以 ```html 代码块输出在对话末尾
- [ ] llms.txt 草稿以 ```text 代码块输出
- [ ] ai-page.md 草稿以 ```text 代码块输出

### 质量对照
**Claude.ai 产出的报告评分与其他平台产出的报告评分应 ≤ 2 分差异**（评分本身是确定性计算，数据源相同即结果相同；差异主要来自抓取内容略有差异）。

如发现差异 > 2 分，检查：
1. fetch_page 是否完整返回了页面内容（Jina Reader 可能对某些动态页面返回不全）
2. search_web 是否返回了摘要（Tavily 免费版结果数量可能略少于 Claude 内置搜索）
3. 模型是否按 SKILL.md 的完整步骤执行（部分平台可能截断长系统提示）
